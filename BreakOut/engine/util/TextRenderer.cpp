...
TextRenderer::TextRenderer(unsigned int screenWidth, unsigned int screenHeight)
{
	...
	m_TextShader = ResourceManager::getShaderProgram("textRenderSP");

	m_TextShader.bind();
	m_TextShader.setUniformMatrix4fv("uProjectionMatrix", projectionMatrix);
	m_TextShader.setUniform1i("uText", 0);
	m_TextShader.unbind();
...
void TextRenderer::load(const char* filepath, unsigned int fontSize)
{
    ...
    else
    {
        ...
        {
            FT_Set_Pixel_Sizes(face, 0, fontSize); // Set size to load glyphs as.
...
void TextRenderer::write(std::string text, float x, float y, float scale, glm::vec3 color)
{
    ...
    m_TextShader.setUniform3f("uTextColor", color);
